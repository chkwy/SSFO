...
base_ssfo_model_name=$ssfo_name
for dataset in "${datasets[@]}"; do
    ...
    echo "Evaluating $base_ssfo_model_name on dataset $dataset"
    python main.py --model-id "$ssfo_model" --eval-dataset "$dataset" --gpu_num 4 >> ../logs/${model_name}_run.log 2>&1
done
...
for dataset in "${datasets[@]}"; do
    ...
    ssfo_metric=$(cat outputs/${dataset}_${ssfo_name}_metrics_final.json | \
                 tr ',' '\n' | grep '"sub_Subspan_EM"' | cut -d':' -f2 | tr -d ' }' | \
                 awk '{printf "%.2f%%", $1*100}')

    printf "%-20s | %-25s | %-25s\n" "$dataset" "$base_metric" "$ssfo_metric"
done
